...
BEGIN_MESSAGE_MAP(CMFCApplication7View, CView)
	...
	ON_WM_CONTEXTMENU()
	ON_WM_RBUTTONUP()
	ON_COMMAND(ID_32772, &CMFCApplication7View::OnCircle)
	ON_WM_LBUTTONDBLCLK()
	ON_COMMAND(ID_32771, &CMFCApplication7View::onshowhide)
END_MESSAGE_MAP()
...
void CMFCApplication7View::OnCircle()
{
	// TODO: 在此添加命令处理程序代码
...
void CMFCApplication7View::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	MyDlg0 *pD = new MyDlg0();
	pD->Create(IDD_DIALOG1);//获取对话框资源
	
	pD->ShowWindow(1);//弹出非模式对话框
	CView::OnLButtonDblClk(nFlags, point);
	
}


void CMFCApplication7View::onshowhide()
{
	// TODO: 在此添加命令处理程序代码
	MyDlg0 *pD = new MyDlg0();
	if (show_hide == 1)
		...
		show_hide = 1;
	pD->ShowWindow(show_hide);//弹出非模式对话框
